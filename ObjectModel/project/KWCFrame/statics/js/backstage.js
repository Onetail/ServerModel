...
async function getAppNumber(type = "app" , img = "statics/images/SOFTWARE/software_requestsbug.png"){
	await $.get(`./get_portfolio?type=${type}`,  (data)=>{
		data = JSON.parse(data)
		array = data
		for(let i = 0 , max = array.length; i < max ; i++){
			array[i] = new Studio_detail(img)
			array[i].getIntroduction = data[i].getIntroduction
			array[i].getDetail = "1"
...
async function sendtoKWCworks_APP(string="")
{
	...
	Image_information_list = await getAppNumber(type="app")
...
async function sendtoKWCworks_WEB(string = "")
{
	...
	Image_information_list.length = 0
	Image_information_list = await getAppNumber(type="web")
...
async function sendtoKWCworks_SOFTWARE(string= "")
{
	...
	Image_information_list = await getAppNumber(type="software")
